...
export const assignBuildingsProduction = {
    ...
    S3Build1: (noHardcap = false): Overlimit => {
        ...
        let multiplier = (vacuum ? 2 : 8e-20) * (3 ** researchesS3[0]) * (2 ** researchesS3[3]) * (3 ** researchesS3[5]) * (1.0000001 ** player.researchesExtra[3][0]) * (calculateEffects.S3Extra1() ** global.accretionInfo.effective) * (1.8 ** player.strangeness[3][0]);
        if (vacuum) {
...
export const calculateMaxLevel = (research: number, stageIndex: number, type: 'researches' | 'researchesExtra' | 'researchesAuto' | 'ASR' | 'strangeness' | 'inflations', addAuto = false) => {
    ...
    if (type === 'researches') {
        ...
        if (stageIndex === 2) {
            ...
            if (research === 0) {
                max = (player.strangeness[3][2] >= 1 ? 20 : 14) + Math.floor(10000000 * calculateEffects.effectiveRank());
            } else if (research === 1) {
            ...
            if (research === 1) {
                ...
                const universes = player.verses[0].true + player.inflation.voidVerses;
                max = universes >= 11 ? 6 : universes >= 9 ? 3 : universes >= 5 ? 2 : 1;
            } else if (research === 4) {
                const universes = player.verses[0].true + player.inflation.voidVerses;
